...
class Markdown {
	...
	constructor(
		...
		//转义
		this.tranRegs = [
			{reg: /\\\\/, to: "<tranBackslash>"},
			{reg: /\\&amp;/, to: "<tranAnd>"},
			{reg: /\\&nbsp;/, to: "<tranSpace>"},
			{reg: /\\&lt;/, to: "<tranLess>"},
			{reg: /\\&gt;/, to: "<tranGreater>"},
			{reg: /\\&#39;/, to: "<tranSingleQuotation>"},
			{reg: /\\&quot;/, to: "<tranDoubleQuotation>"},
			{reg: /\\#/, to: "<tranNumberSign>"},
			{reg: /\\=/, to: "<tranEqual>"},
			{reg: /\\\+/, to: "<tranAdd>"},
			{reg: /\\-/, to: "<tranReduce>"},
			{reg: /\\\*/, to: "<tranAsterisk>"},
			{reg: /\\_/, to: "<tranUnderline>"},
			{reg: /\\`/, to: "<tranBackquote>"},
			{reg: /\\\|/, to: "<tranDelimiter>"},
			{reg: /\\~/, to: "<tranWaveSign>"},
			{reg: /\\\^/, to: "<tranPower>"},
			{reg: /\\!/, to: "<tranExclamatoryMark>"},
			{reg: /\\{/, to: "<tranLeftCurlyBracket>"},
			{reg: /\\}/, to: "<tranRightCurlyBracket>"},
			{reg: /\\\[/, to: "<tranLeftSquareBracket>"},
			{reg: /\\]/, to: "<tranRightSquareBracket>"},
			{reg: /\\\(/, to: "<tranLeftBracket>"},
			{reg: /\\\)/, to: "<tranRightBracket>"},
			{reg: /\\./, to: "<tranDot>"},
		];
		this.tranToRegs = [
			{reg: /<tranBackslash>/, to: "\\"},
			{reg: /<tranAnd>/, to: "&amp;"},
			{reg: /<tranSpace>/, to: "\\&nbsp;"},
			{reg: /<tranLess>/, to: "&lt;"},
			{reg: /<tranGreater>/, to: "&gt;"},
			{reg: /<tranSingleQuotation>/, to: "&#39;"},
			{reg: /<tranDoubleQuotation>/, to: "&quot;"},
			{reg: /<tranNumberSign>/, to: "#"},
			{reg: /<tranEqual>/, to: "="},
			{reg: /<tranAdd>/, to: "+"},
			{reg: /<tranReduce>/, to: "-"},
			{reg: /<tranAsterisk>/, to: "*"},
			{reg: /<tranUnderline>/, to: "_"},
			{reg: /<tranBackquote>/, to: "`"},
			{reg: /<tranDelimiter>/, to: "|"},
			{reg: /<tranWaveSign>/, to: "~"},
			{reg: /<tranPower>/, to: "^"},
			{reg: /<tranExclamatoryMark>/, to: "!"},
			{reg: /<tranLeftCurlyBracket>/, to: "{"},
			{reg: /<tranRightCurlyBracket>/, to: "}"},
			{reg: /<tranLeftSquareBracket>/, to: "["},
			{reg: /<tranRightSquareBracket>/, to: "]"},
			{reg: /<tranLeftBracket>/, to: "("},
			{reg: /<tranRightBracket>/, to: ")"},
			{reg: /<tranDot>/, to: "."},
		];
	...
	markdown() {
		...
		for (let html of htmls) {
			if (i % 2 !== 0) {
				...
				html = replaceImg(html);//必须先找图片再找链接，不然会被a标签解析器先替换
				html = replaceA(html);
